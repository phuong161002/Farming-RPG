...
public class CropDetails
{
    ...
    public Sprite harvestedSprite; // sprite use once harvested
    [ItemCodeDescription]
    public int harvestedTransformItemCode; // if the item transform into other item when harvasted this item code will be populated
    ...
    public int[] cropProducedMinQuantity; // array of minimun quantities produced for the harvested crop
    public int[] cropProducedMaxQuantity; // if max quantity is > min quantity then a random number of crops between min an max are produced
    public int daysToRegrow; // days to regrow next crop or -1 if a single crop

    /// <summary>
    /// return true if the tool item code can be used to harvest this crop, else return false
    /// </summary>
    /// <param name="toolItemCode"></param>
    /// <returns></returns>
    public bool CanUseToolToHarvestCrop(int toolItemCode)
    {
        if(RequireHarvestActionForTool(toolItemCode) == -1)
        {
            return false;
        }
        else
    ...
    public int RequireHarvestActionForTool(int toolItemCode)
    {
