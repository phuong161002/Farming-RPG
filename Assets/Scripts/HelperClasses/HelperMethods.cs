...
public static class HelperMethods
{
    ...
    public static bool GetComponentsAtCursorLocation<T>(out List<T> componentsAtPositionList, Vector3 positionToCheck)
    {
        ...
        T tCompnent = default(T);

        for (int i = 0; i < collider2DArray.Length; i++)
        {
            tCompnent = collider2DArray[i].gameObject.GetComponentInParent<T>();
            if (tCompnent != null)
            {
                found = true;
                componentList.Add(tCompnent);
            }
            else
            {
                tCompnent = collider2DArray[i].gameObject.GetComponentInChildren<T>();
                if (tCompnent != null)
                {
                    found = true;
                    componentList.Add(tCompnent);
