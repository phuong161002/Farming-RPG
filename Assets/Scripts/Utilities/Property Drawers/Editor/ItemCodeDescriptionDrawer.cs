...
public class ItemCodeDescriptionDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (property.propertyType == SerializedPropertyType.Integer)
        {
            ...
            //Draw item code
            var newValue = EditorGUI.IntField(new Rect(position.x, position.y, position.width, position.height / 2), label.text, property.intValue);
    ...
    private string GetItemDescription(int itemCode)
    {
        ...
        List<ItemDetail> itemDetails = so_itemList.itemDetails;

        ItemDetail itemDetail = itemDetails.Find(x => x.itemCode == itemCode);
